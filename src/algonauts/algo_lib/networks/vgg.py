...
class VGG(nn.Module):
    ...
    def __init__(self, features, places, num_classes=1000, init_weights=True):
        super(VGG, self).__init__()
        self.select_feats = ['maxpool1', 'maxpool2',
                             'maxpool3', 'maxpool4', 'maxpool5']
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                ...
                places += [0, 0]
            in_channels = v
    return nn.Sequential(*layers), places


def _vgg(arch, cfg, batch_norm, pretrained, progress, **kwargs):
    if pretrained:
        kwargs['init_weights'] = False
    layers, places = make_layers(cfgs[cfg], batch_norm=batch_norm)
    model = VGG(layers, places, **kwargs)
    if pretrained:
